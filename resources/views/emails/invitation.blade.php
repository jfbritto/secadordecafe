...
<body style="font-family: system-ui, -apple-system, sans-serif; background:#faf6f1; padding:32px; margin:0;">
    <div style="max-width:560px; margin:0 auto; background:#fff; border-radius:16px; padding:36px; border:1px solid #f1e6d6;">
        ...
        <h1 style="color:#5a3a22; margin:0 0 8px; font-size:22px;">Você foi convidado</h1>
        <p style="color:#6e4322; margin:0 0 14px;">Você foi convidado para acessar a fazenda <strong>{{ $farmName }}</strong> como <strong>{{ ucfirst($role) }}</strong>.</p>
        <p style="color:#6e4322; margin:0 0 14px;">Clique no botão abaixo para criar sua conta. Este convite expira em <strong>{{ $expiresAt->format('d/m/Y H:i') }}</strong>.</p>
        ...
        <p style="font-size:12px; color:#a87a47; word-break:break-all;">Ou cole no navegador: {{ $acceptUrl }}</p>
    </div>
</body>
</html>
