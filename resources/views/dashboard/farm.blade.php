        ...
        <div class="divide-y divide-leaf-100">
            @forelse($ultimasMovs as $m)
                <div class="px-6 py-3.5 flex items-center justify-between gap-3">
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-leaf-900 truncate">{{ $m->owner?->nome ?? '—' }}</p>
                        <p class="text-xs text-leaf-500">{{ \App\Support\StatusLabels::movementTipo($m->tipo) }} · {{ \App\Support\StatusLabels::produto($m->produto) }} · {{ $m->occurred_at->format('d/m H:i') }}</p>
                    </div>
