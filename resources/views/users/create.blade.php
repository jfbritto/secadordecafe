...
<div class="max-w-5xl mx-auto">
    <div class="mb-6">
        <p class="text-xs text-leaf-500 mb-1">
            <a href="{{ route('usuarios.index') }}" class="hover:underline">Usuários</a> · <span class="text-leaf-700">Cadastrar</span>
        </p>
        <h1 class="text-2xl font-bold text-leaf-900">Cadastrar novo usuário</h1>
        <p class="text-sm text-leaf-500 mt-1">A pessoa entra direto com o e-mail e senha que você definir agora. Sem precisar de convite por e-mail.</p>
    </div>
    ...
    <form method="POST" action="{{ route('usuarios.store') }}"
        ...
        <div class="bg-white rounded-2xl border border-leaf-100 shadow-sm p-6 sm:p-8">
            <div class="border-b border-leaf-100 pb-5 mb-6">
                <h2 class="text-base font-bold text-leaf-900">Dados de acesso</h2>
                <p class="text-sm text-leaf-500 mt-0.5">Como a pessoa vai entrar no sistema.</p>
            </div>

            <div class="grid sm:grid-cols-2 gap-5 mb-5">
                <div>
                    <label for="name" class="block text-sm font-bold text-leaf-900 mb-2">Nome <span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label for="email" class="block text-sm font-bold text-leaf-900 mb-2">E-mail <span class="text-rose-500">*</span></label>
                    <input id="email" name="email" type="email" required
                           value="{{ old('email') }}"
                           placeholder="exemplo@email.com"
                           class="w-full px-4 py-3 text-base rounded-lg border border-leaf-200 placeholder-leaf-300 focus:border-leaf-500 focus:ring-4 focus:ring-leaf-500/15 outline-none transition">
                    <p class="mt-1.5 text-sm text-leaf-500">Será usado para entrar no sistema.</p>
            ...
            <div class="grid sm:grid-cols-2 gap-5">
                <div>
                    <label for="password" class="block text-sm font-bold text-leaf-900 mb-2">Senha <span class="text-rose-500">*</span></label>
                    <input id="password" name="password" type="password" required
                           placeholder="mínimo 8 caracteres"
                           class="w-full px-4 py-3 text-base rounded-lg border border-leaf-200 placeholder-leaf-300 focus:border-leaf-500 focus:ring-4 focus:ring-leaf-500/15 outline-none transition">
                    <p class="mt-1.5 text-sm text-leaf-500">Letras + números, mínimo 8. Combine com a pessoa pra ela poder mudar depois.</p>
                    @error('password')<p class="mt-2 text-sm font-medium text-rose-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="password_confirmation" class="block text-sm font-bold text-leaf-900 mb-2">Confirmar senha <span class="text-rose-500">*</span></label>
        ...
        <div class="bg-white rounded-2xl border border-leaf-100 shadow-sm p-6 sm:p-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                @foreach(\App\Support\PermissionsMatrix::ROLES as $key => $r)
                    <label class="flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition"
                           :class="role === @js($key) ? 'border-leaf-700 bg-leaf-50' : 'border-leaf-200 hover:bg-leaf-50/50'">
                        <input type="radio" name="role" value="{{ $key }}" x-model="role"
                               class="mt-0.5 w-5 h-5 border-leaf-300 text-leaf-700 focus:ring-leaf-500">
                        <div>
                            <span class="block text-sm font-bold text-leaf-900">{{ $r['label'] }}</span>
                            <span class="block text-xs text-leaf-500 mt-0.5 leading-relaxed">{{ $r['desc'] }}</span>
                        </div>
                    </label>
                @endforeach
            </div>
            ...
            {{-- Matriz dinâmica --}}
            <div class="mt-6">
                <h3 class="text-sm font-bold text-leaf-900 uppercase tracking-wider mb-3">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-leaf-100">
                            @foreach(\App\Support\PermissionsMatrix::MODULES as $module => $mLabel)
                                <tr>
                                    <td class="px-4 py-2 font-medium text-leaf-800">{{ $mLabel }}</td>
                                    @foreach(\App\Support\PermissionsMatrix::ACTIONS as $action => $aLabel)
        ...
        <div class="flex flex-col-reverse sm:flex-row sm:items-center gap-3">
            ...
            <a href="{{ route('usuarios.index') }}" class="text-center sm:text-left px-4 py-3 sm:py-0 text-sm font-semibold text-leaf-600 hover:text-leaf-900">Cancelar</a>
        </div>
    </form>
</div>
@endsection
