...
<div class="bg-white rounded-2xl border border-leaf-100 shadow-sm p-6 mb-6">
    <div class="border-b border-leaf-100 pb-4 mb-5">
        <h2 class="text-base font-bold text-leaf-900">Convidar novo usuário (por e-mail)</h2>
        <p class="text-sm text-leaf-500 mt-0.5">A pessoa recebe um e-mail com link para criar a própria senha. Convite válido por 7 dias. <strong>Use "Cadastrar usuário"</strong> acima se preferir definir a senha agora.</p>
    </div>
    <form method="POST" action="{{ route('convites.store') }}">
        @csrf
        <div class="grid sm:grid-cols-12 gap-4">
            <div class="sm:col-span-6">
                <label for="invite_email" class="block text-sm font-bold text-leaf-900 mb-2">E-mail</label>
            ...
            <div class="sm:col-span-4">
                <label for="invite_role" class="block text-sm font-bold text-leaf-900 mb-2">Permissão</label>
            ...
            <div class="sm:col-span-2 flex items-end">
                <button type="submit" class="w-full px-4 py-3 text-base font-bold text-white bg-leaf-700 hover:bg-leaf-800 rounded-lg transition shadow-sm">Enviar</button>
            </div>
        </div>
    </form>
</div>
        ...
        <tbody class="divide-y divide-leaf-100">
            @foreach($users as $u)
                <tr class="hover:bg-leaf-50/30 transition">
                    <td class="px-6 py-3 text-leaf-900 font-medium">
                        {{ $u->name }}
                        @if($u->id === auth()->id())<span class="ml-1 text-xs text-leaf-500">(você)</span>@endif
                    </td>
                    <td class="px-6 py-3 text-leaf-600">{{ $u->email }}</td>
                    <td class="px-6 py-3">
                        @if($u->id === auth()->id())
                            <span class="text-leaf-700">{{ ucfirst($u->roles->pluck('name')->join(', ') ?: '—') }}</span>
                        @else
                            <form method="POST" action="{{ route('usuarios.role.update', $u) }}" class="inline"
                                ...
                                <select name="role" data-original="{{ $u->roles->pluck('name')->first() }}"
                                        ...
                                        class="px-2 py-1 text-sm rounded-md border border-leaf-200 focus:outline-none focus:ring-2 focus:ring-leaf-500">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-leaf-100">
                @foreach($pendingInvitations as $inv)
                    <tr>
                        <td class="px-6 py-3 text-leaf-900">{{ $inv->email }}</td>
                        <td class="px-6 py-3 text-leaf-600">{{ ucfirst($inv->role) }}</td>
                        <td class="px-6 py-3 text-leaf-500">{{ $inv->expires_at->format('d/m/Y H:i') }}</td>
                        <td class="px-6 py-3 text-right">
