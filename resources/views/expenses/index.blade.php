...
<div class="bg-white rounded-xl border border-leaf-100 shadow-sm p-4 mb-4">
    ...
    <form method="GET">
        <div class="grid sm:grid-cols-4 gap-3 items-end">
            <div>
                <label class="block text-xs font-semibold text-leaf-700 mb-1.5">De</label>
                <input type="date" name="from" value="{{ $from }}"
                       class="w-full px-3 py-2 text-sm rounded-lg border border-leaf-200 focus:outline-none focus:ring-2 focus:ring-leaf-500">
            </div>
            <div>
                <label class="block text-xs font-semibold text-leaf-700 mb-1.5">Até</label>
            ...
            <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-leaf-700 hover:bg-leaf-800 rounded-lg transition">Filtrar período personalizado</button>
        </div>
    </form>
</div>
...
<div class="bg-white rounded-xl border border-leaf-100 shadow-sm p-6 mb-4">
    <div class="flex items-baseline justify-between mb-4">
        <h2 class="text-sm font-bold text-leaf-900 uppercase tracking-wider">Totais por categoria{{ $from || $to ? ' (no período)' : '' }}</h2>
        <div class="text-right">
            <p class="text-[10px] text-leaf-500 uppercase tracking-wider">Total geral</p>
            <p class="text-2xl font-bold text-leaf-700">R$ {{ number_format($totalGeral, 2, ',', '.') }}</p>
        </div>
    </div>

    @if($totals->isEmpty())
        <p class="text-sm text-leaf-500 text-center py-4">Sem lançamentos no período.</p>
    @else
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            @foreach($totals as $row)
                <div class="bg-leaf-50 rounded-lg p-3">
                    <p class="text-[10px] text-leaf-500 uppercase tracking-wider truncate">{{ $row->cat_nome }}</p>
                    <p class="text-base font-bold text-leaf-800 mt-0.5">R$ {{ number_format($row->total, 2, ',', '.') }}</p>
                    <p class="text-[10px] text-leaf-500">{{ $row->qtd }} lançamento(s)</p>
                </div>
            @endforeach
        </div>
    @endif
        ...
        <tbody class="divide-y divide-leaf-100">
            @forelse($expenses as $e)
                <tr class="hover:bg-leaf-50/30 transition">
                    <td class="px-6 py-3 text-leaf-700">{{ $e->data->format('d/m/Y') }}</td>
                    <td class="px-6 py-3 text-leaf-900 font-medium">{{ $e->descricao }}</td>
                    <td class="px-6 py-3 text-leaf-600">{{ $e->categoriaNome() }}</td>
                    <td class="px-6 py-3 text-right font-bold text-leaf-800">{{ number_format($e->valor_total, 2, ',', '.') }}</td>
                    <td class="px-6 py-3 text-right text-sm">
                        @can('update', $e)<a href="{{ route('despesas.edit', $e) }}" class="text-leaf-600 hover:text-leaf-900 hover:underline">editar</a>@endcan
                    </td>
                </tr>
            @empty
                <tr><td colspan="5" class="px-6 py-12 text-center text-leaf-500">Nenhuma despesa.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
