...
<div class="border-b border-coffee-100 pb-5 mb-6">
    ...
    <p class="text-sm text-coffee-500 mt-0.5">Registro do gasto. Você pode informar quantidade e valor unitário, ou só o valor total.</p>
</div>
...
<div class="space-y-5 mb-6">
    <div class="grid sm:grid-cols-2 gap-5">
        <div>
            ...
            <p class="mt-1.5 text-sm text-coffee-500">Quando o gasto foi efetivamente realizado.</p>
            @error('data')<p class="mt-2 text-sm font-medium text-rose-600">{{ $message }}</p>@enderror
        </div>

        <div>
            <label for="expense_category_id" class="block text-sm font-bold text-coffee-900 mb-2">
            ...
            <p class="mt-1.5 text-sm text-coffee-500">
                Não viu? <a href="{{ route('despesas.categorias.create') }}" class="text-coffee-700 font-semibold hover:underline">Cadastrar nova</a>.
            </p>
    ...
    <div>
        <label for="descricao" class="block text-sm font-bold text-coffee-900 mb-2">
        ...
        <p class="mt-1.5 text-sm text-coffee-500">O que foi este gasto. Aparece na listagem e nos relatórios.</p>
        @error('descricao')<p class="mt-2 text-sm font-medium text-rose-600">{{ $message }}</p>@enderror
    </div>
</div>

<div class="border-b border-coffee-100 pb-5 mb-6">
    <h2 class="text-base font-bold text-coffee-900">Valores</h2>
    ...
    <div>
        ...
        <div class="relative">
            ...
            <input id="valor_unitario" type="number" step="0.01" min="0" inputmode="decimal" name="valor_unitario"
                   ...
                   placeholder="0,00"
                   class="w-full pl-12 pr-4 py-3 text-base rounded-lg border border-coffee-200 placeholder-coffee-300 focus:border-coffee-500 focus:ring-4 focus:ring-coffee-500/15 outline-none transition">
        </div>
    ...
    <div>
        ...
        <div class="relative">
            ...
            <input id="valor_total" type="number" step="0.01" min="0.01" inputmode="decimal" name="valor_total" required
                   ...
                   placeholder="0,00"
                   class="w-full pl-12 pr-4 py-3 text-base rounded-lg border-2 border-coffee-300 placeholder-coffee-300 focus:border-coffee-500 focus:ring-4 focus:ring-coffee-500/15 outline-none transition font-semibold">
        </div>
        <p class="mt-1.5 text-xs text-coffee-500">Total pago</p>
...
<div class="mb-6">
    ...
    <textarea id="observacoes" name="observacoes" rows="3"
              ...
              class="w-full px-4 py-3 text-base rounded-lg border border-coffee-200 placeholder-coffee-300 focus:border-coffee-500 focus:ring-4 focus:ring-coffee-500/15 outline-none transition">{{ old('observacoes', $E?->observacoes) }}</textarea>
    <p class="mt-1.5 text-sm text-coffee-500">Opcional. Útil pra encontrar a despesa depois.</p>
</div>
