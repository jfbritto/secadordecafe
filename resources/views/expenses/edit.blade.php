...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <p class="text-xs text-leaf-500 mb-1">
            <a href="{{ route('despesas.index') }}" class="hover:underline">Despesas</a> · <span class="text-leaf-700">Editar</span>
        </p>
        <h1 class="text-2xl font-bold text-leaf-900">Editar despesa</h1>
    </div>

    <form method="POST" action="{{ route('despesas.update', $expense) }}" class="bg-white rounded-2xl border border-leaf-100 shadow-sm p-6 sm:p-8">
        @method('PUT')
        ...
        <div class="mt-6 bg-white rounded-2xl border border-rose-200 shadow-sm p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-sm text-rose-600 mt-0.5">Remover esta despesa do sistema. A ação não pode ser desfeita.</p>
                </div>
