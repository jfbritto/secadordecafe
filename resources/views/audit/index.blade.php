...
<div class="mb-6">
    ...
    <p class="text-sm text-coffee-500 mt-0.5">Histórico de tudo que foi cadastrado, editado ou excluído na fazenda. Útil pra conferir o que aconteceu e quando.</p>
</div>
...
<div x-data="{ open: false, diff: [], title: '', subtitle: '' }">
    <div class="bg-white rounded-2xl border border-coffee-100 shadow-sm overflow-hidden">
        @forelse($activities as $a)
            @php
                ...
                $eventCls = match($h['event']) {
                    ...
                    default   => ['bg' => 'bg-coffee-100', 'text' => 'text-coffee-700', 'verb' => $h['action']],
                };
            @endphp

            <div class="px-4 sm:px-6 py-4 border-b border-coffee-100 last:border-b-0 flex items-start gap-4 hover:bg-coffee-50/30 transition">
                {{-- Ícone da entidade --}}
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm text-coffee-900 leading-snug">
                        <strong>{{ $h['actor'] }}</strong>
                        <span class="text-coffee-600">{{ $eventCls['verb'] }}</span>
                        <span class="text-coffee-500">·</span>
                        <span class="font-medium">{{ $h['entity_label'] }}</span>
                        <strong class="text-coffee-800">{{ $h['subject_name'] }}</strong>
                    </p>
                    <p class="text-xs text-coffee-500 mt-1">
                ...
                <div class="flex-shrink-0">
                    @if($h['has_diff'])
                        <button type="button"
                                ...
                                class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-coffee-700 bg-coffee-50 hover:bg-coffee-100 rounded-lg transition border border-coffee-200">
        ...
        @empty
            <div class="px-6 py-16 text-center text-coffee-500">
    ...
    <div x-show="open" x-cloak
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col"
            ...
            <div class="px-6 py-4 border-b border-coffee-100 flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <h2 class="text-lg font-bold text-coffee-900 truncate" x-text="title"></h2>
                    <p class="text-xs text-coffee-500 mt-0.5" x-text="subtitle"></p>
                </div>
                <button @click="open = false" class="flex-shrink-0 p-2 rounded-lg text-coffee-500 hover:bg-coffee-50 hover:text-coffee-900">
            ...
            <div class="overflow-y-auto p-6 flex-1">
                <template x-if="diff.length === 0">
                    <p class="text-sm text-coffee-500 text-center py-8">Sem alterações registradas.</p>
                </template>
                <template x-if="diff.length > 0">
                    <div class="space-y-3">
                        <template x-for="(row, i) in diff" :key="i">
                            <div class="border border-coffee-100 rounded-xl p-4">
                                <p class="text-xs font-bold text-coffee-500 uppercase tracking-wider mb-2" x-text="row.field"></p>
            ...
            <div class="px-6 py-4 border-t border-coffee-100 flex justify-end">
                <button @click="open = false" class="px-5 py-2 text-sm font-semibold text-white bg-coffee-700 hover:bg-coffee-800 rounded-lg transition">Fechar</button>
            </div>
        </div>
    </div>
</div>
