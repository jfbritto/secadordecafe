...
<div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
    <div>
        <p class="text-xs text-leaf-500 mb-1"><a href="{{ route('despesas.index') }}" class="hover:underline">Despesas</a></p>
        <h1 class="text-2xl font-bold text-leaf-900">Categorias de despesa</h1>
        ...
        <tbody class="divide-y divide-leaf-100">
            @forelse($categories as $cat)
                <tr class="hover:bg-leaf-50/30 transition">
                    <td class="px-6 py-3 font-semibold text-leaf-900">{{ $cat->nome }}</td>
                    <td class="px-6 py-3 text-right text-leaf-700">{{ $cat->expenses_count }}</td>
                    <td class="px-6 py-3">
                        @if($cat->ativo)
                    ...
                    <td class="px-6 py-3 text-right text-sm">
                        @can('update', $cat)
                            <a href="{{ route('despesas.categorias.edit', $cat) }}" class="text-leaf-600 hover:text-leaf-900 hover:underline">editar</a>
                        @endcan
                    </td>
                </tr>
            @empty
                <tr><td colspan="4" class="px-6 py-12 text-center text-leaf-500">Nenhuma categoria cadastrada.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
