...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <p class="text-xs text-leaf-500 mb-1">
            ...
            <span class="text-leaf-700">{{ $category->nome }}</span>
        </p>
        <h1 class="text-2xl font-bold text-leaf-900">Editar categoria</h1>
    </div>

    <form method="POST" action="{{ route('despesas.categorias.update', $category) }}" class="bg-white rounded-2xl border border-leaf-100 shadow-sm p-6 sm:p-8">
        @method('PUT')
        ...
        <div class="flex flex-col-reverse sm:flex-row sm:items-center gap-3 pt-6 border-t border-leaf-100">
            ...
            <a href="{{ route('despesas.categorias.index') }}" class="text-center sm:text-left px-4 py-3 sm:py-0 text-sm font-semibold text-leaf-600 hover:text-leaf-900">Cancelar</a>
        </div>
    </form>
