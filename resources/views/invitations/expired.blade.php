...
<div class="bg-white rounded-2xl shadow-xl shadow-leaf-700/5 border border-leaf-100 p-8 text-center">
    ...
    <h1 class="text-2xl font-bold text-leaf-900">Convite expirado</h1>
    <p class="text-sm text-leaf-500 mt-2">Este convite expirou em {{ $invitation->expires_at->format('d/m/Y H:i') }}.</p>
    <p class="text-sm text-leaf-600 mt-3">
        Entre em contato com a administradora da fazenda <strong>{{ $invitation->farm?->nome }}</strong>
        para receber um novo convite.
    </p>
    <a href="{{ route('login') }}" class="inline-block mt-6 text-sm font-semibold text-leaf-700 hover:underline">Ir para login</a>
</div>
@endsection
