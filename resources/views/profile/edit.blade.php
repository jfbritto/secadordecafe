...
<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-coffee-900">Meu perfil</h1>
        <p class="text-sm text-coffee-500 mt-1">Seus dados pessoais e senha de acesso à Roça Nossa.</p>
    </div>

    {{-- Card: dados pessoais --}}
    <form method="POST" action="{{ route('perfil.update') }}" class="bg-white rounded-2xl border border-coffee-100 shadow-sm p-6 sm:p-8 mb-6">
        @csrf
        @method('PUT')

        <div class="border-b border-coffee-100 pb-5 mb-6">
            <h2 class="text-base font-bold text-coffee-900">Dados pessoais</h2>
            <p class="text-sm text-coffee-500 mt-0.5">Como você aparece dentro do sistema.</p>
        </div>
        ...
        <div class="space-y-5">
            ...
            <div>
                <label for="email" class="block text-sm font-bold text-coffee-900 mb-2">
                    E-mail <span class="text-rose-500">*</span>
                </label>
                <input id="email" type="email" name="email" required maxlength="180"
                       value="{{ old('email', $user->email) }}"
                       class="w-full px-4 py-3 text-base rounded-lg border border-coffee-200 placeholder-coffee-300 focus:border-coffee-500 focus:ring-4 focus:ring-coffee-500/15 outline-none transition">
                <p class="mt-1.5 text-sm text-coffee-500">Usado para entrar no sistema e receber e-mails da plataforma.</p>
                @error('email')<p class="mt-2 text-sm font-medium text-rose-600">{{ $message }}</p>@enderror
            </div>
        </div>

        <div class="flex justify-end pt-6 mt-6 border-t border-coffee-100">
            <button type="submit" class="px-6 py-3 text-base font-bold text-white bg-coffee-700 hover:bg-coffee-800 rounded-lg transition shadow-sm">
                Salvar dados
            </button>
        </div>
    </form>
    ...
    {{-- Card: senha --}}
    <form method="POST" action="{{ route('perfil.senha.update') }}" class="bg-white rounded-2xl border border-coffee-100 shadow-sm p-6 sm:p-8">
        @csrf
        @method('PUT')

        <div class="border-b border-coffee-100 pb-5 mb-6">
            <h2 class="text-base font-bold text-coffee-900">Senha</h2>
            <p class="text-sm text-coffee-500 mt-0.5">Pra trocar a senha, informe a atual e a nova duas vezes.</p>
        </div>

        <div class="space-y-5">
            <div>
                <label for="current_password" class="block text-sm font-bold text-coffee-900 mb-2">
            ...
            <div class="grid sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input id="password" type="password" name="password" required autocomplete="new-password" minlength="8"
                           class="w-full px-4 py-3 text-base rounded-lg border border-coffee-200 focus:border-coffee-500 focus:ring-4 focus:ring-coffee-500/15 outline-none transition">
                    <p class="mt-1.5 text-sm text-coffee-500">Mínimo 8 caracteres, com letras e números.</p>
