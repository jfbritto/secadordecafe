...
<div class="border-b border-leaf-100 pb-5 mb-6">
    <h2 class="text-base font-bold text-leaf-900">Identificação da área</h2>
    <p class="text-sm text-leaf-500 mt-0.5">Como esta área (talhão/lote) será exibida nas listas e nas secagens.</p>
</div>
...
<div class="space-y-5 mb-6">
    ...
    <div>
        ...
        <textarea id="observacoes" name="observacoes" rows="3"
                  ...
                  class="w-full px-4 py-3 text-base rounded-lg border border-leaf-200 placeholder-leaf-300 focus:border-leaf-500 focus:ring-4 focus:ring-leaf-500/15 outline-none transition">{{ old('observacoes', $A?->observacoes) }}</textarea>
    </div>
</div>
...
<div class="mb-6" x-data="{
    ...
    <div class="grid sm:grid-cols-2 gap-5 mb-3">
        <div>
            <label for="latitude" class="block text-sm font-bold text-leaf-900 mb-2">Latitude</label>
            <input id="latitude" type="text" name="latitude" x-model="lat" readonly
                   placeholder="—"
                   class="w-full px-4 py-3 text-base rounded-lg border border-leaf-200 bg-leaf-50/50 text-leaf-700 outline-none">
            @error('latitude')<p class="mt-2 text-sm font-medium text-rose-600">{{ $message }}</p>@enderror
        </div>
        <div>
            <label for="longitude" class="block text-sm font-bold text-leaf-900 mb-2">Longitude</label>
    ...
    <div class="flex flex-wrap gap-2 items-center">
        <button type="button" @click="capturar()" :disabled="loading"
                class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-leaf-700 hover:bg-leaf-800 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition shadow-sm">
        ...
        <button type="button" @click="limpar()" x-show="lat && lng" x-cloak
                class="text-sm font-semibold text-leaf-600 hover:text-leaf-900 hover:underline">
            Limpar
        </button>
        <a x-show="lat && lng" x-cloak :href="'https://www.google.com/maps?q=' + lat + ',' + lng" target="_blank" rel="noopener"
           class="inline-flex items-center gap-1 text-sm font-semibold text-leaf-700 hover:underline">
            Ver no mapa
    ...
    <p class="mt-2 text-sm text-leaf-500" x-show="status" x-text="status" x-cloak></p>
    <p class="mt-2 text-sm text-leaf-500" x-show="!status">
...
<div class="border-b border-leaf-100 pb-5 mb-6">
    ...
    <p class="text-sm text-leaf-500 mt-0.5">Controle se esta área aparece no menu ao criar uma nova secagem.</p>
</div>
...
<div class="mb-6">
    <label class="flex items-start gap-3 p-4 rounded-lg border-2 border-leaf-200 cursor-pointer hover:bg-leaf-50/50 transition has-[:checked]:border-leaf-500 has-[:checked]:bg-leaf-50">
        ...
        <div>
            <span class="block text-sm font-bold text-leaf-900">Área ativa</span>
