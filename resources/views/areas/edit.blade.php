...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <p class="text-xs text-leaf-500 mb-1">
            ...
            <a href="{{ route('areas.show', $area) }}" class="hover:underline">{{ $area->nome }}</a> ·
            <span class="text-leaf-700">Editar</span>
        </p>
        <h1 class="text-2xl font-bold text-leaf-900">Editar área</h1>
    </div>

    <form method="POST" action="{{ route('areas.update', $area) }}" class="bg-white rounded-2xl border border-leaf-100 shadow-sm p-6 sm:p-8">
        @method('PUT')
        @include('areas._form')
        ...
        <div class="flex flex-col-reverse sm:flex-row sm:items-center gap-3 pt-6 border-t border-leaf-100">
            ...
            <a href="{{ route('areas.show', $area) }}" class="text-center sm:text-left px-4 py-3 sm:py-0 text-sm font-semibold text-leaf-600 hover:text-leaf-900">Cancelar</a>
        </div>
    </form>
        ...
        <div class="mt-6 bg-white rounded-2xl border border-rose-200 shadow-sm p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-sm text-rose-600 mt-0.5">Remove a área permanentemente. Só funciona se não houver secagens vinculadas.</p>
                </div>
                <form method="POST" action="{{ route('areas.destroy', $area) }}"
                      data-confirm="Excluir esta área?"
                      data-confirm-text="Só é possível se não houver secagens vinculadas. Pra tirar de circulação sem perder o histórico, desmarque 'Área ativa' em vez de excluir."
