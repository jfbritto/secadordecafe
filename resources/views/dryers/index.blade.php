...
<div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
    <div>
        <h1 class="text-2xl font-bold text-coffee-900">Secadores</h1>
        <p class="text-sm text-coffee-500 mt-0.5">Equipamentos de secagem da fazenda.</p>
    </div>
    @can('create', App\Models\Dryer::class)
        <a href="{{ route('secadores.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-white bg-coffee-700 hover:bg-coffee-800 rounded-lg transition shadow-sm">
        ...
        <tbody class="divide-y divide-coffee-100">
            @forelse($dryers as $d)
                <tr class="hover:bg-coffee-50/30 transition">
                    <td class="px-6 py-3 font-semibold text-coffee-900">{{ $d->nome }}</td>
                    <td class="px-6 py-3 text-coffee-600">{{ $d->modelo ?? '—' }}</td>
                    <td class="px-6 py-3 text-right text-coffee-700">
                        {{ $d->capacidade_kg ? number_format($d->capacidade_kg, 0, ',', '.') . ' kg' : '—' }}
                    </td>
                    <td class="px-6 py-3 text-right text-coffee-700">{{ $d->secagens_count }}</td>
                    <td class="px-6 py-3">
                        @if($d->ativo)
                    ...
                    <td class="px-6 py-3 text-right text-sm">
                        @can('update', $d)
                            <a href="{{ route('secadores.edit', $d) }}" class="text-coffee-600 hover:text-coffee-900 hover:underline">editar</a>
                        @endcan
                    </td>
                </tr>
            @empty
                <tr><td colspan="6" class="px-6 py-12 text-center text-coffee-500">
                    Nenhum secador cadastrado.
                    @can('create', App\Models\Dryer::class)
                        <a href="{{ route('secadores.create') }}" class="text-coffee-700 font-semibold hover:underline">Cadastrar agora</a>.
                    @endcan
                </td></tr>
            @endforelse
        </tbody>
